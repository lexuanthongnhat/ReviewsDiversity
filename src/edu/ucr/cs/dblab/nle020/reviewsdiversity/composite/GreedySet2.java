...
public class GreedySet2 {
	...
	public GreedySet2(int k, float threshold,
...
//		printResult();	
	...
	private void checkResult(List<FullPair> topK,
		...
		if (verifyingCost != (long) result.getFinalCost()) 
			System.err.println("Wrong cost, verying cost: " + verifyingCost + ", cost: " + result.getFinalCost());
		...
		if (verifyingCost != (long) result.getFinalCost()) 
			System.err.println("Wrong cost 2, verying cost: " + verifyingCost + ", cost: " + result.getFinalCost());
	}
...
//						fullPairSet.increaseBenefit(fullPairToCSPair.get(potentialHost).calculateRootDistance());
		...
		initNumPotentialUsefulCoverWithThreshold(result, fullPairSets);
	}

	private void initNumPotentialUsefulCoverWithThreshold(TopPairsResult result, Collection<FullPair> fullPairSets) {
		...
		result.setNumPotentialUsefulCoverWithThreshold(numPotentialUsefulCoverWithThreshold);
	}
