...
for label, num in label_to_num.iteritems():
    ...
    for i in range(num): 
        train_arrays[train_index] = model.docvecs[label_to_train_tag[label] + str(i)]
...
num_sentences = 244
filename = "test-sentences"
predict_full = False
if predict_full:
    num_sentences = 110615
    filename = "all-sentences" 

test_arrays = numpy.zeros((num_sentences, dimension)) 
file_path = "./reviews/" + filename + ".txt"
inferred_vector_save = "./reviews/" + filename
with codecs.open(file_path, encoding="utf-8") as test:
    percent_to_notify = 5
...
#test_arrays = numpy.load(inferred_vector_save + ".npy")
print test_arrays
...
numpy.save(inferred_vector_save, test_arrays)
print "Saved sentences' inferred vector to {}.npy".format(inferred_vector_save)
