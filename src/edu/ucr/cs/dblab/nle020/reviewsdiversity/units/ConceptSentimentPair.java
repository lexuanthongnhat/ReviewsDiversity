...
public class ConceptSentimentPair {
  ...
  private boolean isSentimentCover(ConceptSentimentPair p, float threshold) {
    if (deweys.contains(SnomedGraphBuilder.ROOT_DEWEY)
        || p.deweys.contains(SnomedGraphBuilder.ROOT_DEWEY)
        || this.getId().equals(Constants.ROOT_CUI)
        || p.getId().equals(Constants.ROOT_CUI)
    ) {
      return true;
    }
    return Math.abs(this.sentiment - p.sentiment) <= threshold;
  }
